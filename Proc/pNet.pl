...
##	(See Lu Lu et.al 2018 PNet: bridging the gap between drugs and diseases reveals novel pharmacological pathways 
##	for more details)
####################################################################################################################################
...
## A example for running this software: perl pNet.pl -dr ../Data/DrugBank.Drug.Info.txt -di ../Data/OMIM.Disease.Info.txt -n ../Data/HumanNet.txt -dt ../Data/DrugBank.All.Target.Entrez.IDs.txt -dg ../Data/gene_EntrezIDs_disease_OMIMIDs.txt -o ../Result/ -gmin 4 -gmax 5 -gstep 0.5 -opt SDrTDi

##	get options from command
my ($drFile,$diFile,$netFile,$dtFile,$dgFile,$outPath,$gammaMin,$gammaMax,$gammaStep,$optimethod,$help);			
...
pNet($drFile,$diFile,$netFile,$dtFile,$dgFile,$outPath,\@gammas,$optimethod);

sub pNet{
	my($drFile,$diFile,$netFile,$dtFile,$dgFile,$outPath,$gammas,$optimethod) =@_;
	...
	foreach my $gamma (@{$gammas}){
		...
		mkdir($outDir);
		LPModeler($drFile,$diFile,$netFile,$dtFile,$dgFile,$outDir,$gamma,$optimethod);
	}
}
...
sub LPModeler{
	my($drFile,$diFile,$netFile,$dtFile,$dgFile,$outDir,$gamma,$optimethod) = @_;
	my(%drHash,%diHash,%dtHash,%dgHash);
	...
	if($optimethod eq "SDrTDi"){
		foreach my $drug (keys %drHash){
			my $stat = lpsolveModeling($drug,\%drHash,\%diHash,\%dtHash,\%dgHash,$netFile,$outDir,$gamma,$optimethod);
			if($stat == 0){
		...
		foreach my $disease (keys %diHash){
			my $stat = lpsolveModeling($disease,\%drHash,\%diHash,\%dtHash,\%dgHash,$netFile,$outDir,$gamma,$optimethod);
			if($stat == 0){
